...
#ROI_POINTS = np.array([[410, 300], [550, 300], [750, 460], [230, 460]])
...
# Variable to track the lane status
lane_status = 'right'  # Starting at the right lane
...
def detect(img, session):
    global object_tracking, closest_object_in_roi, lane_status, lane_status_2, lane_status_3, lane_status_4, lane_status_5, coned
    ...
    if closest_object_in_roi and closest_object_in_roi[0] == 'Human':

        print(f"Closest object in ROI: {closest_object_in_roi[0]}, Distance: {closest_object_in_roi[1]:.2f} m")
        if closest_object_in_roi[1] < 18 and closest_object_in_roi[1] > 8:
            print("Applying brakes")
            publisher_brakes.publish(0.7)
            publisher_velocity.publish(0.0)
        elif closest_object_in_roi[1] < 10:
            print("Emergency braking!")
            publisher_brakes.publish(1)
            publisher_velocity.publish(0.0)


    elif closest_object_in_roi and closest_object_in_roi[0] == 'Cone' and coned == 0:
        ...
        print("Slowing down and changing lane")
        publisher_velocity.publish(0.07)
        # Decrease speed
        print("Steering left for lane change")
        publisher_steering.publish(4.0)
        
    elif lane_status == 1 and  lane_status_2 == 1 and lane_status_3 == 1 and lane_status_4 == 1 and lane_status_5 == 1 and closest_object_in_roi is None:
        print("Adjusting new lane..")
        publisher_velocity.publish(0.05)
        publisher_steering.publish(-15.5)
        publisher_brakes.publish(0.0)
        rospy.sleep(3.0)
        lane_status = 0
        lane_status_2 = 0
        lane_status_3 = 0
        ...
        publisher_steering.publish(0.0)
        publisher_velocity.publish(0.2)
        publisher_brakes.publish(0.0)
...
if __name__ == "__main__":
    rospy.init_node("Image_node_2")
    rospy.Subscriber("/image", Image, image_callBack, queue_size=1)
    publisher_velocity = rospy.Publisher("/cmd_vel", Float64, queue_size=64)
    publisher_brakes = rospy.Publisher("/brakes", Float64, queue_size=64)
    publisher_steering = rospy.Publisher("/SteeringAngle", Float64, queue_size=64)
